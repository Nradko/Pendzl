...
import { getLocalApiProviderWrapper } from './setup/helpers';

const name = 'My Token';
const symbol = 'MTKN';
const decimals: number = 12;

const MAX_U128 = new BN(2).pow(new BN(128)).subn(1);
...
describe.only('ERC4626', function () {
  ...
  it('decimals overflow', async function () {
    for (const offset of [244, 250, 255]) {
      ...
      await expect(vault.query.tokenDecimals()).to.be.revertedWithError('Overflwo');
    }
  });
  ...
  describe('limits', async function () {
    ...
    it('reverts on deposit() above max deposit', async function () {
      ...
      await expect(vault.withSigner(holder).tx.deposit(maxDeposit!.addn(1), recipient.address)).to.be.revertedWithError('ExceededMaxDeposit');
    });
    ...
    it('reverts on mint() above max mint', async function () {
      ...
      await expect(vault.withSigner(holder).tx.deposit(maxMint!.addn(1), recipient.address)).to.be.revertedWithError('ExceededMaxMint');
    });
    ...
    it('reverts on withdraw() above max withdraw', async function () {
      ...
      await expect(vault.withSigner(holder).tx.withdraw(maxWithdraw!.addn(1), recipient.address, holder.address)).to.be.revertedWithError(
        'ExceededMaxWithdraw',
      );
    });
    ...
    it('reverts on redeem() above max redeem', async function () {
      ...
      await expect(vault.withSigner(holder).tx.redeem(maxRedeem!.addn(1), recipient.address, holder.address)).to.be.revertedWithError(
        'ExceededMaxRedeem',
      );
    });
  });
  ...
  for (const offset of [0, 6, 18]) {
    const parseToken = (token: number) => new BN(token).muln(10 ** decimals);
    const parseShare = (share: number) => new BN(share).muln(10 ** (decimals + offset));

    const virtualAssets = new BN(1);
    const virtualShares = new BN(10 ** offset);
    ...
    describe(`offset: ${offset}`, function () {
      ...
      beforeEach(async function () {
        ...
        await token.tx.tMint(holder.address, MAX_U128.divn(2)); // 50% of maximum
      ...
      describe('empty vault: no assets & no shares', function () {
        it('status', async function () {
          await expect(vault.query.totalAssets()).to.haveOkResult(0n);
        });
  ...
  it('multiple mint, deposit, redeem & withdrawal', async function () {
    ...
    await expect(tx).to.emitEvent(token, 'Transfer', { from: vault.address, to: alice.address, value: 2428 });
    ...
    await expect(tx).to.emitEvent(token, 'Transfer', { from: vault.address, to: bruce.address, value: 8001 });
